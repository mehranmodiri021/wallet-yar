...
object Bip39WordList {
    val words: List<String> by lazy {
        rawWordList.trim().split("\\s+".toRegex())
    }

    val wordToIndex: Map<String, Int> by lazy {
        words.mapIndexed { index, word -> word to index }.toMap()
    }

    fun isValidWord(word: String): Boolean = wordToIndex.containsKey(word.lowercase().trim())

    fun findSuggestions(query: String, maxResults: Int = 10): List<String> {
        val q = query.lowercase().trim()
        if (q.isEmpty()) return emptyList()

        val prefixMatches = words.filter { it.startsWith(q) }
        if (prefixMatches.isNotEmpty()) {
            return prefixMatches.take(maxResults)
        }

        // Fuzzy matches using Levenshtein distance
        return words.map { it to levenshteinDistance(q, it) }
            .filter { it.second <= 2 }
            .sortedBy { it.second }
            .take(maxResults)
            .map { it.first }
    ...
    // Official BIP-39 English Wordlist (Exactly 2048 words)
    private const val rawWordList = """
